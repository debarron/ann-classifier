...
void ayuda(char *p){
	...
	fprintf(stdout, "## Entrenar\n%s -e RED datos_entrenamiento lerning-rate (Error | Max_Epocas)\n", p);
	fprintf(stdout, "-------------------------------------------------------------------\n");
...
int main(int argc, char **argv){
	...
	float *paa, acum;
	int n_paa, j, indice, d, k, offset, gap;
	
	FILE *p_red, *p_datos;
	bd_t *bd_entrenamiento, *bd_paa;
	...
	if(op_parada == 0) entrena_x_epocas_paa(red, bd_entrenamiento, learning_rate, epocas_max);
